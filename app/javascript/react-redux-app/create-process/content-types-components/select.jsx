...
const Select = ({ block, editable, updatedElement, updateParentContentBlock, onClick, onChange, onKeyDown, myRef }) => {
  const contentType = block.content_type

  if (editable) {
    return(
      <span key="editableSpan" ref={this.props.myRef} contentEditable className={`block-input large bold ${block.content_type.style}`} key="outlineTitle" onKeyDown={this.props.onKeyDownHandler} onChange={this.props.onChange} suppressContentEditableWarning={true}>block.text</span>
    )
  }else{
